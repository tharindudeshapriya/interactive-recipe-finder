...
const CategoryCarousel = ({ title, description, recipes, loading }) => {
    if (loading && (!recipes || recipes.length === 0)) {
        return <div className="mb-24 h-64 bg-neutral-light/5 animate-pulse rounded-3xl border border-neutral-light/10" />;
    }

    if (!loading && (!recipes || recipes.length === 0)) return null;

    return (
        <CarouselContainer title={title} description={description} autoScroll={true}>
            {loading ? (
